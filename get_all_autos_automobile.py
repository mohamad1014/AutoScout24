...
path_to_marche_modelli_urls = os.path.join(folder_dir, 'marche', 'marche_modelli_urls.json')
...
# Get the different marche
with open(path_to_marche_modelli_urls, 'r') as file:
    marche_modelli_urls = json.load(file)
    print(f"path to json file:{path_to_marche_modelli_urls} with {len(marche_modelli_urls)} elements")

ii=1
for marche_modelli_url in marche_modelli_urls[::-1]:
    print(f"Executing {marche_modelli_url}")
#    for xx in range(11, 502, 10):
    car_URLs=[]
    for page in range(1, 501):
        try:
            url = f'{marche_modelli_url.split("?")[0]}/page-{page}?b=data&d=DESC'
    ...
    if len(car_URLs_unique) > 0:
        print(f'{country} | Site {page} | {len(car_URLs_unique)} new URLs')
    ...
    print(f'done {marche_modelli_url} gonna go the next') # This does the range
    # sleep(30)
